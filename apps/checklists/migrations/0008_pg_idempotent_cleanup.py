...
SQL = """
-- Make legacy drops safe if columns/indexes are already gone

-- ChecklistRun legacy fields (drop only if present)
...
-- ChecklistTemplate legacy fields (drop only if present)
...
-- Ensure timestamps exist (harmless if already there)
